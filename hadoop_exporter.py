...
class CommonMetricCollector(MetricCol):
    '''
    ...
    def __init__(self, cluster, url, component, service):
        ...
        @param service: Service name. e.g. "namenode", "resourcemanager", "mapreduce".
        '''
        MetricCol.__init__(self, cluster, url, component, service)
        self._common_metrics = {}
        self._hadoop_common_metrics = {}
        self._common_metrics_list = utils.get_file_list("common")
        for i in range(len(self._common_metrics_list)):
            self._common_metrics.setdefault(self._common_metrics_list[i], utils.read_json_file("common", self._common_metrics_list[i]))
            self._hadoop_common_metrics.setdefault(self._common_metrics_list[i], {})
    ...
    def collect(self):
        ...
        self._setup_common_labels()

        # add metric value to every metric.
        self._get_common_metrics(beans)

        # yield all metrics
        for i in range(len(self._common_metrics_list)):
            service = self._common_metrics_list[i]
            for metric in self._hadoop_common_metrics[service]:
                self._hadoop_common_metrics[service][metric]
        return self._hadoop_common_metrics

    def _setup_common_labels(self):

        if "JvmMetrics" in self._common_metrics:
            for metric in self._common_metrics['JvmMetrics']:
                snake_case = "jvm_" + re.sub('([a-z0-9])([A-Z])', r'\1_\2', metric).lower()
                if 'Mem' in metric:
                    name = snake_case + 'ebibytes'
                    label = ["cluster", "mode"]
                    if "Used" in metric:
                        ...
                        key = name
                        descriptions = self._common_metrics['JvmMetrics'][metric]
                elif 'Gc' in metric:
                    label = ["cluster", "type"]
                    if "GcCount" in metric:
                        key = "jvm_gc_count"
                    ...
                    key = snake_case
                    descriptions = self._common_metrics['JvmMetrics'][metric]
                self._hadoop_common_metrics['JvmMetrics'][key] = GaugeMetricFamily(self._prefix + key,
                                                                                   descriptions,
                                                                                   labels=label)

    def _get_common_metrics(self, beans):
        for i in range(len(beans)):
            if 'JvmMetrics' in beans[i]['name']:
                # 记录JvmMetrics在bean中的位置
                if 'JvmMetrics' in self._common_metrics:
                    labels = [].append(self._cluster)
                    for metric in self._common_metrics['JvmMetrics']:
                        name = "jvm_" + re.sub('([a-z0-9])([A-Z])', r'\1_\2', metric).lower()
                        if 'Mem' in metric:
                            if "NonHeap" in metric:
                                labels.append("nonheap")
                            elif "MemHeap" in metric:
                                labels.append("heap")
                            elif "Max" in metric:
                                labels.append("max")
                            ...
                            if "GcCount" in metric:
                                ...
                                key = name
                            if "ParNew" in metric:
                                labels.append("ParNew")
                            elif "ConcurrentMarkSweep" in metric:
                                labels.append("ConcurrentMarkSweep")
                            elif "Warn" in metric:
                                labels.append("Warn")
                            elif "Info" in metric:
                                labels.append("Info")
                            ...
                            key = "jvm_threads_state_total"
                            labels.append(metric.split("Threads")[1])
                        elif 'Log' in metric:
                            key = "jvm_log_level_total"
                            labels.append(metric.split("Log")[1])
                        else:
                            key = name
                        
                        self._hadoop_common_metrics['JvmMetrics'][key].add_metric(labels,
                                                                                  beans[i][metric] if metric in beans[i] else 0)
...
class NameNodeMetricsCollector(MetricCol):
    ...
    def __init__(self, cluster):
        ...
        self._merge_list = self._file_list + utils.get_file_list("common")
